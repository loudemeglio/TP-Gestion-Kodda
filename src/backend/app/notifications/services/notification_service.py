...
class NotificationService:
    CONFIRMED_TITLE = "Compra confirmada"
    CONFIRMED_MESSAGE = (
    ...
    def on_order_confirmed(db: Session, order: Order) -> None:
        """Genera notificaciones para cada vendedor involucrado cuando la orden queda confirmada."""
        ...
        for seller_id in seller_ids:
            NotificationRepository.create(
                ...
                Notification(
                    user_id=seller_id,
                    title=NotificationService.CONFIRMED_TITLE,
                    message=NotificationService.CONFIRMED_MESSAGE,
                    is_read=False,
                    order_id=order.id,
                ),
            )
